...
const createCollege = async function (req, res) {
  ...
  try {
    ...
    if (!validator.isValidElem(name))
      return res
        .status(400)
        .send({ status: false, message: "name is require " });
    if (!validator.isValidName(name))
      return res
        .status(400)
        .send({ status: false, message: "name should be in alphabets" });
    ...
    if (findName)
      return res
        ...
        .send({
          status: false,
          message: "the college name is already present",
        });

    // -----------------------checking the full college name validation---------------------------
    if (!validator.isValidElem(fullName))
      return res
        .status(400)
        .send({ status: false, message: "full name is require" });
    if (!validator.isValidName(fullName))
      return res
        .status(400)
        .send({ status: false, message: " full name should be in alphabets" });

    // ---------------------checking the logo link validation--------------------------------------
    if (!validator.isValidElem(logoLink))
      return res
        .status(400)
        .send({ status: false, message: "logoLink is require" });
    if (!validator.isValidLogo(logoLink))
      return res
        .status(400)
        .send({ status: false, message: "It should be a link " });
    ...
    return res
      .status(201)
      .send({ status: true, msg: "college cretaed", data: saveData });
  } catch (err) {
  ...
  try {
    ...
    if (!collegeName) {
      return res
        ...
        .send({
          status: true,
          msg: "collegeName is missing or you left empty",
        });
    }
    ...
    if (!collegeData) {
      return res
        ...
        .send({
          status: true,
          msg: "collegeData you have provided is incorrect",
        });
    }
    let details = await internModel
