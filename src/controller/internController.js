...
const createintern = async function (req, res) {
  ...
  try {
    ...
    if (!validator.isValidElem(email))
      return res
        .status(400)
        .send({ status: false, message: "email is require " });
    if (!validator.isValidEmail(email))
      return res
        .status(400)
        .send({ status: false, message: "email must in correct formate" });
    ...
    if (!validator.isValidElem(mobile))
      return res
        .status(400)
        .send({ status: false, message: "mobile number is require " });
    if (!validator.isValidmobile(mobile))
      return res
        .status(400)
        .send({ status: false, message: "mobile number must be 10 digits" });
    let findmobile = await internModel.findOne({ mobile });
    if (findmobile)
      return res
        .status(400)
        .send({ status: false, message: "mobile number is already present" });
    ...
    if (!newintern)
      return res
        ...
        .send({
          status: false,
          message: "No intern found with proper college name",
        });
    ...
    return res
      ...
      .send({
        status: true,
        message: "intern created succesfuly",
        data: saveData,
      });
  } catch (err) {
